...
;(async () => {
  ...
  let c = await chat()
  expect('EN main menu', await c.send('hi'), 'SchoolPay', '💳', 'Pay Fees', 'Check fee balance', 'Academic results', 'Badili lugha', 'Payment Statements')
  expect('EN pay → child list', await c.send('1'), 'Clarence Kioko Mueni - School XYZ', 'Enter admission number', 'Help', '* Back')
  expect('EN email prompt', await c.send('1'), 'email address', 'payment receipt', 'skip')
  expect('EN payment progress', await c.send('skip'), 'Payment progress', 'Lipa pole pole', 'Pay all Term 2 fees')
  ...
  // real email + back navigation with '*'
  c = await chat()
  await c.send('hi'); await c.send('1'); await c.send('1')
  expect('EN real email accepted', await c.send('parent@gmail.com'), 'Payment progress')
  ...
  // admission number entry from the child list
  c = await chat()
  await c.send('hi'); await c.send('1')
  expect('EN admission prompt', await c.send('2'), 'Admission Number', 'ADM/2025/001')
  expect('EN admission → email', await c.send('ADM/2025/001'), 'email address', 'skip')

  // balance flow (statement + PDF option)
  c = await chat()
  await c.send('hi'); await c.send('2')
  expect('EN balance screen', await c.send('1'), `${mon} balance: Ksh 8,000`, 'Term 2 full balance: Ksh 20,000', 'Payment Statement', 'Balance PDF copy')
  expect('EN statement shared', await c.send('1'), 'Payment Statement shared', 'Total paid: Ksh 6,500', '* Home')
  c = await chat()
  await c.send('hi'); await c.send('2'); await c.send('1')
  expect('EN balance PDF asks email', await c.send('2'), 'email address', 'balance PDF')

  // payment statements (menu option 5)
  c = await chat()
  await c.send('hi'); await c.send('5')
  expect('EN stmt month picker', await c.send('1'), 'Payment Statements', 'Select a period', 'May 2026', 'June 2026')
  expect('EN stmt view + PDF offer', await c.send('1'), 'Payment Statement', 'KES', 'PDF')
  expect('EN stmt decline PDF → menu', await c.send('n'), 'Pay Fees')

  // results flow (+ transcript offer)
  c = await chat()
  await c.send('hi'); await c.send('3')
  expect('EN results months', await c.send('1'), 'results ready', 'May/2026', 'June/2026', 'All', '* Back')
  expect('EN results shared', await c.send('1'), 'results shared', 'Maths', 'transcript', '* Home')
  expect('EN transcript decline → menu', await c.send('n'), 'Pay Fees')

  // help (child listed)
  c = await chat()
  await c.send('hi'); await c.send('1')
  expect('EN help menu', await c.send('3'), 'Clarence Kioko Mueni is not my child', 'I have another child in this school')
  expect('EN help thanks', await c.send('1'), 'Thank you', 'school office', '* Home')

  // ════════ WhatsApp/SMS — Kiswahili ════════
  c = await chat()
  await c.send('hi')
  expect('SW main menu', await c.send('4'), 'Lipa karo', 'Karo idaiwayo', 'Matokeo ya elimu', 'Back to english', 'Risiti za malipo')
  expect('SW child list', await c.send('1'), 'Clarence Kioko Mueni - School XYZ', 'Andika nambari ya usajili', 'Msaada', '* Rudi')
  expect('SW email prompt', await c.send('1'), 'barua pepe', 'skip')
  expect('SW payment progress', await c.send('skip'), 'Lipa pole pole', 'Lipa salio lote la muhula wa pili')
  ...
  expect('USSD child list', await u('1'), '1. Clarence Kioko Mueni - School XYZ', '2. Enter admission number', '3. Help')
  ...
  expect('USSD admission prompt', await u('1*2'), 'Enter admission number')
  expect('USSD admission → progress', await u('1*2*ADM/2025/001'), 'Payment progress', 'Lipa pole pole')
  expect('USSD admission → amount', await u('1*2*ADM/2025/001*1'), `${mon} balance Ksh 8,000`)
  ...
  expect('USSD help menu', await u('1*3'), 'is not my child', 'I have another child in this school')
  expect('USSD help thanks', await u('1*3*1'), 'END', 'school office')
